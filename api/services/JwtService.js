...
const JwtService = {
  ...
  verificaPermessi: async (decoded, livelloRichiesto, scopoRichiesto, ambitoRichiesto) => {
    ...
    const utenteHaRichiestoScopoCorretto = !scopoRichiesto || scopoRichiesto.every(s => scopi.includes(s));
    const utenteHaAutorizzazioneAScopiRichiesti = !scopoRichiesto || scopoRichiesto.every(s => scopiUtenteAttivi.includes(s));
    if (!utente || !utente.attivo || !utenteHaAutorizzazioneAScopoToken|| !utenteHaRichiestoScopoCorretto || !utenteHaAutorizzazioneAScopiRichiesti || !utente.ambito
      || utente.ambito.ambito !== ambito || (ambitoRichiesto && ambito !== ambitoRichiesto) || (livelloRichiesto !== undefined && utente.livello < livelloRichiesto) || livello !== utente.livello) {
      return false;
    }
